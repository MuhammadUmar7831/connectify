...
const authenticate = async (req: authRequest, res: Response, next: NextFunction) => {
    try {
        ...
        const decode = jwt.verify(token, process.env.JWT_SECRET as string) as _JwtPayload;
        const userEmail: string = decode.email as string;
        const query: string = `SELECT * FROM Users WHERE Email = ?`;
        connection.query(query, [userEmail], (err: QueryError | null, result: RowDataPacket[]) => {
